        ...
        for phase in ['train', 'val']:
            ...
            for idx, data in enumerate(data_loaders[phase]):
                ...
                with torch.set_grad_enabled(phase == 'train'):
                    ...
                    loss1 = criterion(x_classes, labels_classes)
                    loss2 = criterion(x_species, labels_species)
                ...
                corrects_classes_and_species = torch.zeros_like(preds_species)
                corrects_classes_and_species[preds_classes == labels_classes] = 1
                corrects_classes_and_species[preds_species != labels_species] = 0
                corrects_classify += torch.sum(corrects_classes_and_species)

            epoch_loss = running_loss / len(data_loaders[phase].dataset)
            Loss_list[phase].append(epoch_loss)

            epoch_acc_Multi = corrects_classify.double() / len(data_loaders[phase].dataset)
            epoch_acc = epoch_acc_Multi

            Accuracy_list_Multi[phase].append(100 * epoch_acc_Multi)
            print('{} Loss: {:.4f}  Acc_Multi: {:.2%}'.format(phase, epoch_loss,epoch_acc_Multi))

            if phase == 'val' and epoch_acc > best_acc:

                best_acc = epoch_acc_Multi
                best_model_wts = copy.deepcopy(model.state_dict())
                print('Best val Multis Acc: {:.2%}'.format(best_acc))

    model.load_state_dict(best_model_wts)
    torch.save(model.state_dict(), 'best_model.pt')
    print('Best val Multi Acc: {:.2%}'.format(best_acc))
...
def visualize_model(model):
    ...
    with torch.no_grad():
        for i, data in enumerate(data_loaders['val']):
            inputs = data['image']
            labels_classes = data['classes'].to(device)
            labels_species = data['Multi'].to(device)
            ...
            plt.title('predicted Multi: {}\n ground-truth Multi:{}'.format(CLASSES[preds_classes],CLASSES[labels_classes]))
            plt.title('predicted Multi: {}\n ground-truth Multi:{}'.format(SPECIES[preds_species],SPECIES[labels_species]))
            plt.show()
